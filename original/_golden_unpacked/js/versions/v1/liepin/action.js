(function () {
    // console.log(111);
    function checkRepeat (html,type,type2) {
        ...
        if (type) {
            ...
            utils.message.sendMsg({
                type: 'request.api.checkRepeat2',
                settings: {
                    type: 'post',
                    data: {data: JSON.stringify({html: [html],url: url})}
                }
            });
        } else {
            utils.message.sendMsg({
                type: 'request.api.checkRepeat',
                settings: {
                    type: 'post',
                    data: {data: JSON.stringify({html: [html],url: url})}
                }
    ...
    setTimeout(function () {
        catchHtmlToCheckRepeat();
    }, 2000);
    function catchHtmlToCheckRepeat (type) {
        var headLP = '';
        var linkLP = '';
    ...
    utils.message.register({
        type: 'checkRepeatMore', callback: function (msg) {
            console.log(666666);
            catchHtmlToCheckRepeat('click');
        }
    });  
