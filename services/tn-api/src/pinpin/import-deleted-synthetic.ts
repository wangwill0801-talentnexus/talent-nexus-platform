...
async function readFixture(): Promise<Awaited<ReturnType<PinpinSourceAdapter['readPage']>>[number]> {
  ...
  try {
    const page = await adapter.readPage(43176, 1);
    const snapshot = page[0];
    if (!snapshot || snapshot.externalCandidateId !== '43177') throw new Error('Approved synthetic source candidate was not found.');
    return snapshot;
  } finally {
    await adapter.close();
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const first = await reconciler.reconcileDeletedSynthetic(await readFixture());
    const second = await reconciler.reconcileDeletedSynthetic(await readFixture());
    const integrity = await reconciler.verifyIntegrity(second.candidateId, second.candidateCode, '43177');
    process.stdout.write(`${JSON.stringify({
      sourceCandidate: '43177',
      firstRun: first,
      secondRun: second,
...
main().catch((error: unknown) => {
  process.stderr.write(`PINPIN_DELETED_SYNTHETIC_IMPORT_FAILED:${failureCategory(error)}\n`);
  process.exitCode = 1;
});
