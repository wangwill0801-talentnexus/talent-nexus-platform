...
if (window == window.top) {
    ...
    window.CanvasRenderingContext2D.prototype.fillText = function (text, ...args) {
        ...
        fillTextEndChecker = setTimeout(function () {
            ...
            window.zhipinPluginWasmMod.destroy();
            window.zhipinPluginWasmMod = null;
...
window.zhipinWasmDecrypt = async function (encryptString) {
    ...
    if (!window.zhipinPluginWasmMod) {
        try {
            //wasmMod = await import('https://static.zhipin.com/assets/zhipin/wasm/resume/wasm_canvas-1.0.2-5018.js');
            window.zhipinPluginWasmMod = await import(chrome.runtime.getURL('js/versions/v1/zhipin/wasm_canvas-1.0.2-5018.js'));
        } catch (e) {
        ...
        // 初始化并调用start
        try {
            await window.zhipinPluginWasmMod.default('https://static.zhipin.com/assets/zhipin/wasm/resume/wasm_canvas_bg-1.0.2-5018.wasm');
        } catch (e) {
    ...
    window.zhipinPluginWasmMod.start(
        container,
        content,
        encryptString,
        null,
